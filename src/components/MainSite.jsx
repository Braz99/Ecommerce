    ...
    area.innerHTML = dados.map(
      (jogo) =>
        `<li id=${jogo.id} key=${jogo.id}>
          ...
          <img src=${"assets/" + jogo.image} alt=${
          "imagem do jogo " + jogo.name
        } />

          <h3 id='${jogo.id}preco_item'>R$ ${jogo.price
          .toFixed(2)
          .replace(".", ",")}</h3>
  ...
  return (
    <main id="main">
      <select id="filtro" onChange={() => filtrar()}>
        <option value="padrao" defaultValue>
          Filtrar
        </option>
