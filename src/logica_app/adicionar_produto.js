function adicionarProduto(id) {
  ...
  let preco_item = document.getElementById(id + 'preco_item').innerText.split(' ')

  while (true) {
    let item = localStorage.getItem(contador)

    if (!item) {
      localStorage.setItem(contador, JSON.stringify({ 'nome': nome_item, 'preco': parseFloat(preco_item[1]), 'imagem': imagem_item, 'status': 'adicionar' }))
